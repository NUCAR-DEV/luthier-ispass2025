...
def main():
    ...
    print(len(benchmark_cfg["InstrCount"]["benchmarks"]))
    for bench in benchmark_cfg["InstrCount"]["benchmarks"]:
        for programming_model in benchmark_cfg["InstrCount"]["programming_models"]:
            ...
            with open(os.path.join(benchmark_folder, RESULT_PKLE_FILE_NAME), 'rb') as f:
                ...
                plot_entries[programming_model].append(
                    data_point["Instrumented Kernel Runtime (us)"] / data_point["Un-instrumented Kernel Runtime (us)"])
                print(len(plot_entries[programming_model]))
    ...
    for idx, bench in enumerate(benchmark_cfg["InstrCount"]["benchmarks"]):
        ...
        for programming_model in benchmark_cfg["InstrCount"]["programming_models"]:
            print(
                f"\t-{programming_model}: {plot_entries[programming_model][idx]:.2f} (~= 10^{math.log10(plot_entries[programming_model][idx]):.2f})")

    print("Average overhead:")
    for programming_model in benchmark_cfg["InstrCount"]["programming_models"]:
        print(
            f"{programming_model}: {sum(plot_entries[programming_model]) / len(plot_entries[programming_model]):.2f}")

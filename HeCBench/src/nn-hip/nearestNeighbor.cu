...
void FindNearestNeighbors(
  ...
  LatLong* d_locations;
  float* d_distances;
  hipMalloc((void**)&d_locations, numRecords * sizeof(LatLong));
  hipMalloc((void**)&d_distances, numRecords * sizeof(float));
  hipMemcpy(d_locations, locations.data(), numRecords * sizeof(LatLong), hipMemcpyHostToDevice);
  ...
  for (int i = 0; i < repeat; i++)
    hipLaunchKernelGGL(nn, gridDim, blockDim, 0, 0, numRecords, lat, lng, d_locations, d_distances);
  ...
  hipMemcpy(distances, d_distances, numRecords * sizeof(float), hipMemcpyDeviceToHost);
  hipFree(d_locations);
  hipFree(d_distances);
}
